    ...
    <div style={{ height: 'calc(100vh - 140px)', display: 'flex', flexDirection: 'column' }}>
      <div className="card" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ flexShrink: 0 }}>
          <div className="flex justify-between items-center mb-4">
            ...
            <button className="btn btn-primary" style={{ height: '42px' }} onClick={() => setIsAddModalOpen(true)}><Plus size={18} /> Thêm ca mới</button>
          </div>
        </div>
      ...
      {isAddModalOpen && (
        <ModalOverlay onClose={() => setIsAddModalOpen(false)}>
          <div className="flex justify-between items-center mb-6" style={{ flexShrink: 0 }}>
            <h2 className="text-xl font-bold">Thêm Ca Khung Giờ Mới</h2>
          ...
          <div style={{ flexShrink: 0, paddingTop: '1rem', marginTop: 'auto', borderTop: '1px solid var(--color-border)' }}>
            <button className="btn btn-primary w-full" onClick={() => setIsAddModalOpen(false)}>Tạo Ca Mới</button>
          </div>
        </ModalOverlay>
      )}
      ...
      {editSlot && (
        <ModalOverlay onClose={() => setEditSlot(null)}>
          <div className="flex justify-between items-center mb-6" style={{ flexShrink: 0 }}>
            <h2 className="text-xl font-bold">Chỉnh Sửa Ca Khung Giờ</h2>
      ...
      {deleteSlot && (
        <ModalOverlay onClose={() => setDeleteSlot(null)}>
          <div className="flex justify-between items-center mb-6" style={{ flexShrink: 0 }}>
            <h2 className="text-xl font-bold text-danger">Xóa Ca Khung Giờ</h2>
